...
class Board:
    ...
    def enumerar_opciones_legales(
        self, player: "Player", dados: List[int]
    ) -> List["OpcionMovimiento"]:
        """
        Generates all legal move options for a player given dice values.
        ...
        player : Player
            The player making the move
        dados : List[int]
            List of dice values to use

        Returns
        -------
        List[OpcionMovimiento]
            List of all possible legal move options
        """
        ...
        if not isinstance(dados, list):
            raise ValueError("dados must be a list")

        if not dados:
            return []

        # Handle doubles - expand to 4 dice
        dice_to_use = list(dados)
        if len(dice_to_use) == 2 and dice_to_use[0] == dice_to_use[1]:
            dice_to_use = dice_to_use * 2

        # Generate all possible sequences using a fresh board state
        sequences = self._generar_secuencias_movimiento(
            player, dice_to_use, [], self
        )

        # Convert sequences to OpcionMovimiento objects and remove duplicates
        opciones_map = {}
        for secuencia in sequences:
            hash_sec = self._calcular_hash_secuencia(player, secuencia)
            if hash_sec not in opciones_map:
                puntaje = len(secuencia)  # Simple scoring: prefer using more dice
                opciones_map[hash_sec] = OpcionMovimiento(
                    secuencia=secuencia, hash_tablero=hash_sec, puntaje=puntaje
                )

        return list(opciones_map.values())
    ...
    def _generar_secuencias_movimiento(
        ...
        # Base case: no more dice to use
        if not dados_restantes:
            return [secuencia_actual] if secuencia_actual else []

        # Get possible moves for the first die
        die_value = dados_restantes[0]
        remaining_dice = dados_restantes[1:]

        # Generate possible moves for this die value
        possible_moves = self._generar_movimientos_posibles(
            player, die_value, tablero_actual
        )

        # If no moves possible with this die
        if not possible_moves:
            # Try to continue with remaining dice
            if remaining_dice:
                return self._generar_secuencias_movimiento(
                    player, remaining_dice, secuencia_actual, tablero_actual
                )
            # No more dice and no moves possible
            return [secuencia_actual] if secuencia_actual else []
        ...
        for move in possible_moves:
            ...
            tablero_simulado = copy.deepcopy(tablero_actual)

            # Apply the move to the simulated board
            try:
                tablero_simulado._aplicar_paso_movimiento(player, move)
            except (ValueError, IndexError):
                # If move fails, skip it
                continue

            # Build new sequence with this move
            nueva_secuencia = secuencia_actual + [move]

            # Recursively generate sequences with remaining dice
            if remaining_dice:
                sub_sequences = self._generar_secuencias_movimiento(
                    player, remaining_dice, nueva_secuencia, tablero_simulado
                )
                all_sequences.extend(sub_sequences)
            else:
                # No more dice, this sequence is complete
                all_sequences.append(nueva_secuencia)

        # Also consider stopping here if we have at least one move
        if secuencia_actual:
            all_sequences.append(secuencia_actual)

        return all_sequences if all_sequences else []
