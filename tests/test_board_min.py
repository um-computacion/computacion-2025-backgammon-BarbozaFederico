...
def test_board_initialization():
    ...
    assert all(isinstance(point, list) for point in board.triangulo)
    assert all(len(point) == 0 for point in board.triangulo)


def test_board_display(capsys):
    """Testea que el método display imprime las 24 puntas."""
    board = Board()
    board.display()
